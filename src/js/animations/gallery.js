// Talks photo gallery. Greyscale filmstrip with four GSAP flourishes:
//   1. Flip "expand"  — the clicked thumbnail morphs into the full lightbox image.
//   2. Stagger reveal — tiles rise + fade in one-by-one as the strip scrolls in.
//   3. Glide prev/next — images crossfade + slip sideways in the travel direction.
//   4. Draggable strip — grab-and-flick with inertia (overflow only).
...
export function initGallery({ reduced = false } = {}) {
  ...
  // ---------- draggable / throwable strip (full-motion only) ----------
  let dragged = false;
  ...
  thumbs.forEach((btn, i) =>
    btn.addEventListener("click", (e) => {
      if (dragged) { dragged = false; e.preventDefault(); return; } // it was a flick, not a tap
      open(i);
    })
  );
